...
function StaffLogin({ navigate, showToast }) {
  ...
  const submit = async () => {
    ...
    setSubmitting(true)
    try {
      await login({ staffId: id, password })
  ...
  return (
    <section className="page">
      ...
      <div className="feature-grid">
        ...
        <FeatureCard title="Room Management" text="Create rooms and control manual availability." action="Manage Rooms" onClick={() => navigate('/staff/rooms')} />
        <FeatureCard title="Amenities Management" text="Create, update, and remove billable amenities." action="Manage Amenities" onClick={() => navigate('/staff/amenities')} />
...
function RoomManagement({ rooms, refreshRooms, showToast }) {
  ...
  const create = async () => {
    ...
    try {
      ...
      setForm({ roomNumber: '', category: 'Classic', bedrooms: 1, pricePerNight: '' })
      await refreshRooms()
      showToast('Room created.')
    } catch {
      showToast('Unable to save room.')
    } finally {
      setSaving(false)
    }
  }
  ...
  const toggle = async (room) => {
    try {
      await roomsApi.updateAvailability(room._id, !room.isAvailable)
      await refreshRooms()
      showToast(`Room ${room.roomNumber} updated.`)
    } catch {
      showToast('Unable to update room availability.')
    }
  }

  return (
    <section className="page">
      <PageTitle eyebrow="Staff" title="Room Management" text="Manage manual availability and add rooms to the inventory." />
      <div className="panel">
        <div className="form-grid">
          <Input label="Room Number" type="number" value={form.roomNumber} onChange={(v) => setForm({ ...form, roomNumber: v })} />
          <Select label="Category" value={form.category} options={categories.slice(1)} onChange={(v) => setForm({ ...form, category: v })} />
          <Input label="Bedrooms" type="number" value={form.bedrooms} onChange={(v) => setForm({ ...form, bedrooms: v })} />
          <Input label="Price Per Night" type="number" value={form.pricePerNight} onChange={(v) => setForm({ ...form, pricePerNight: v })} />
        </div>
        <button className="primary-button" type="button" disabled={saving} onClick={create}>{saving ? 'Saving...' : 'Add Room'}</button>
      </div>
      <div className="table-card">
        <table>
          <thead><tr><th>Room</th><th>Category</th><th>Bedrooms</th><th>Rate</th><th>Status</th><th></th></tr></thead>
          <tbody>
            {rooms.map((room) => (
              <tr key={room._id}>
                ...
                <td>{room.bedrooms}</td>
                <td>{formatCurrency(room.pricePerNight)}</td>
                <td><Badge status={room.isAvailable ? 'Vacant' : 'Unavailable'} /></td>
                <td><button className="table-action" type="button" onClick={() => toggle(room)}>{room.isAvailable ? 'Disable' : 'Enable'}</button></td>
              </tr>
            ))}
          </tbody>
        </table>
...
function AmenityManagement({ amenities, setAmenities, showToast }) {
  ...
  const refresh = async () => setAmenities(await amenitiesApi.list())

  const save = async () => {
    if (saving) return
    setSaving(true)
    try {
      await amenitiesApi.create({ ...form, code: form.code.toUpperCase(), price: Number(form.price) })
      setForm({ code: '', name: '', price: '', type: 'PerBooking', category: 'Convenience' })
      await refresh()
      showToast('Amenity created.')
    } catch {
      showToast('Unable to save amenity.')
    } finally {
      setSaving(false)
    }
  }
  ...
  const remove = async (code) => {
    try {
      await amenitiesApi.remove(code)
      await refresh()
      showToast('Amenity removed.')
    } catch {
      showToast('Unable to remove amenity.')
    }
  }

  return (
    <section className="page">
      <PageTitle eyebrow="Staff" title="Amenities Management" text="Manage add-ons used during payment." />
      <div className="panel">
        <div className="form-grid">
          ...
          <Select label="Type" value={form.type} options={['PerBooking', 'PerNight', 'PerGuest']} onChange={(v) => setForm({ ...form, type: v })} />
          <Select label="Category" value={form.category} options={amenityCategories} onChange={(v) => setForm({ ...form, category: v })} />
        </div>
        <button className="primary-button" type="button" disabled={saving} onClick={save}>{saving ? 'Saving...' : 'Add Amenity'}</button>
      </div>
      <div className="table-card">
        <table>
          <thead><tr><th>Code</th><th>Name</th><th>Price</th><th>Type</th><th>Category</th><th></th></tr></thead>
          <tbody>
            {amenities.map((amenity) => (
              <tr key={amenity.code}>
                ...
                <td><button className="table-action danger" type="button" onClick={() => remove(amenity.code)}>Delete</button></td>
              </tr>
            ))}
          </tbody>
        </table>
